...
import com.example.presentation.screens.headlines.HeadLinesIntent
...
fun ItemArticle(item: ArticlesModel, headLinesViewModel: HeadLinesViewModel) {
    ...
    val favArticles = headLinesViewModel.state.value.favoritesArticlesState.articles
    Card(
        modifier = Modifier
            .padding(16.dp)
            .fillMaxWidth()
                ...
                Icon(
                    ...
                    modifier = Modifier
                        ...
                        if (favArticles.map { it.title }.contains(item.title))
                            item.title?.let {
                                headLinesViewModel.sendIntent(HeadLinesIntent.DeleteFavArticle(it))
                            }
                        else
                            headLinesViewModel.sendIntent(HeadLinesIntent.SaveFavArticle(item))
                                   },
